...
class TasmanianLegislation(Scraper):
    ...
    async def get_index_reqs(self) -> set[Request]:
        ...
        return {
            Request(f'https://www.legislation.tas.gov.au/projectdata?ds=EnAct-BrowseDataSource&start=1&count=5000&expression=PrintType={type}+AND+Year={year}?+AND+PitValid=@pointInTime({pit})&collection=')
